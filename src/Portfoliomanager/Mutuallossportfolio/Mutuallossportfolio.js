...
const Mutualloss = () => { 
  ...
  const stockData = {
    navChange: -0.12,
    gainWeight: -0.48,  // Negative for testing
    investmentCostWeight: 1170,
    latestValueWeight: 1165.60,
    unrealizedGain: -0.38  // Negative for testing
  };

  // Utility function to render negative values only
  const renderNegativeValue = (value) => {
    return value < 0 ? `${value}%` : null; // Only return the value if it's negative
  };

  // Utility function to determine if the value is negative and apply red color
  const getPercentageClass = (value) => {
    return value < 0 ? "negative-value" : ""; // Apply red color for negative, empty for non-negative
  };
        ...
        <div className="portfolio-account-stock-header">
          ...
          <div className="portfolio-account-stock-controls">
            ...
            <div className="portfolio-account-stock-filters">
              ...
              <button className="filter-button" style={{ background: "#24b676", color: "white" }}>
                Losers
              </button>
            </div>
          ...
          <tbody>
            <tr>
              ...
              <td className={getPercentageClass(stockData.navChange)}>
                {renderNegativeValue(stockData.navChange)}
              </td>
              <td className={getPercentageClass(stockData.gainWeight)}>
                {renderNegativeValue(stockData.gainWeight)}
              </td>
              <td>4</td>
              <td>{renderNegativeValue(stockData.investmentCostWeight)}</td>
              <td>{renderNegativeValue(stockData.latestValueWeight)}</td>
              <td className={getPercentageClass(stockData.unrealizedGain)}>
                {renderNegativeValue(stockData.unrealizedGain)}
              </td>
              <td>-</td>
            </tr>
